...
async function main() {
  ...
  if (!session) {
    console.error('No Plaud session found. Run `plaud login` first.');
    process.exit(1);
  }
  ...
  const recordingIdSchema: any = { recording_id: z.string().describe('The recording ID') };
  ...
  server.tool('plaud_get_transcript', 'Get the transcript of a Plaud recording by ID.', recordingIdSchema, async (params: any) => {
  ...
  server.tool('plaud_get_recording_detail', 'Get full details of a Plaud recording including metadata and transcript.', recordingIdSchema, async (params: any) => {
    const detail = await client.getRecording(params.recording_id);
  ...
  server.tool('plaud_get_mp3_url', 'Get a temporary download URL for the MP3 version of a recording.', recordingIdSchema, async (params: any) => {
