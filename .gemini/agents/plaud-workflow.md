...
4. Transcrição Híbrida:
   - Tente transcrição via API primeiro.
   - Se não houver, use ask_user para perguntar se quer Whisper Local ou Online.
   - Se Whisper Local: A saída deve conter minutagem/secundagem `[MM:SS]` e a identificação automática de falantes `[Speaker X]` (Diarização) no início de cada mudança de fala.
5. Salvar no Obsidian (usando obsidian-template.md) e registrar no SQLite via scripts/db_manager.py.
6. Análise Estruturada: Ao processar/resumir uma transcrição já existente, DEVE-SE sempre usar o `obsidian-template.md` e colar a transcrição bruta COMPLETA na seção final da nota gerada. Após criar a nota, atualize o SQLite (`analyzed: 1` e `final_path`).
